...
**Blocked on:** 🔴 **A decision on the purge migration.** Batch 1 complete; Batch 2 part-done (impact dialog + bulk Cancel built and verified). `public.tasks` has RLS on with **no DELETE policy**, so a purge deletes 0 rows and raises nothing — measured. The control is hidden and the action refuses loudly until a `tasks_delete` policy exists (rule R1). Avatars (2.3) still to do. Also still open: **a decision on [REDESIGN-PLAN §9](REDESIGN-PLAN.md)** — what the supplied `CNI-AI-Digital-Task-Board.html` is meant to become. It sat in the repo root, referenced by no planning document, through the whole redesign. Also awaiting permission: **persisting board order** needs a migration (§8.5). `SUPABASE_STORAGE_KEY` and `CRON_SECRET` are set as of Session 17. Resend's sending domain is still deferred, and it is what the email-change verification link waits on.
