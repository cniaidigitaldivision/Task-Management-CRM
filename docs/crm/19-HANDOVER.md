...
| **A `next build` beside a live `next dev` corrupts the dev server** | Tailwind's scanner started emitting garbled class names — `--content-max` came out as `--�G�\1 nt-max` — and every route answered 500 with *"Parsing CSS source code failed"* in `app/globals.css`, a file nobody had touched. ⚠️ `NEXT_DIST_DIR` does **not** prevent this; it was added for exactly that and was measured failing. | **Stop the dev server before building.** Recovery: kill the `next dev` process, `rm -rf .next` (all of it, not just `dev/`), restart. Then check the compiled stylesheet has no U+FFFD before believing it. |
| **A transformed ancestor traps `position: fixed`** | The export dialog opened BELOW THE FOLD and the owner reported "nothing appears". The shell's `.reveal-children` animation ends on `transform: none` with `fill-mode: both`, which computes to an identity matrix — enough to make it the containing block, so `fixed inset-0` filled the 1540px page container, not the 712px viewport. | Use `components/ui/dialog.tsx`. It is a native `<dialog>`, which the browser puts in the TOP LAYER, outside any containing block. Never hand-roll a `fixed inset-0` overlay inside page content. |
