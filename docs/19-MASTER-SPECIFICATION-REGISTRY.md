...
### Contradiction sweep — Session 07 (Phase 1, Step 2)

Found while building the data foundation. Same rule: recorded with a resolution **before** the affected code was written.

| # | Conflict | Resolution |
|:--:|---|---|
| **C-13** | **Doc 09 §4 recommends Supabase Auth.** Doc 16 §4–§6 specifies our own identity stack: Argon2id hashes in `auth_identities.password_hash`, our own `sessions` table with device binding, role-scoped TTL, refresh rotation and reuse detection, a 3-attempt lockout, our own `mfa_factors`, `recovery_codes` and `invitations`. Doc 19 §6 lists all of those as Phase 1 tables and doc 20 §2 gives the `auth` module ownership of them. Both cannot be true — Supabase Auth (GoTrue) owns its own users, password hashes (bcrypt) and session model. | ✅ **Doc 16 wins — §1 of this registry gives it "every auth, session, and recovery decision."** The CRM implements **its own authentication** against its own `users` / `auth_identities` / `sessions` tables. **Supabase provides Postgres, Storage and Realtime only — not Auth.** Doc 09 §4 is superseded. Rationale: GoTrue cannot express a 3-attempt account lock (FR-155a), Argon2id (FR-147), device-bound sessions with reuse detection (FR-150), step-up re-auth (FR-149), or MFA-after-email-code recovery (FR-155b). Layering doc 16's session model on top of GoTrue would mean two session systems and two sources of truth for identity — exactly the fragility doc 20 exists to prevent. |
| **C-14** | **C-13 removes `auth.uid()`.** Doc 04 §5 and doc 16 §7 require RLS as the second, independent enforcement layer, but every Supabase RLS example keys off `auth.uid()`, which only exists for GoTrue sessions. With our own auth there is no database-visible identity. | ✅ **RLS keys off a transaction-local identity the server sets.** Every request opens a transaction and issues `SET LOCAL ROLE cni_app; SET LOCAL app.user_id = '<uuid>'`. Policies read it through `app.current_user_id()` / `app.current_user_role()`. `cni_app` is a `NOBYPASSRLS` role, so policies actually bind — unlike `postgres`, which bypasses them. **Fail-closed:** with no `app.user_id` set, `app.current_user_id()` is NULL and every policy predicate is false. Contract documented in `lib/db/README.md`. |
| **C-15** | **The pre-authentication bootstrap gap.** Verifying a password requires reading `auth_identities` *before* an identity exists, so the sign-in path cannot satisfy C-14's `app.user_id`. | ✅ **A narrow `SECURITY DEFINER` surface in schema `app`, and nothing else.** Pre-auth operations (identity lookup by email, recording a login attempt, creating a session) are a small, named, individually reviewed set of definer functions. **The application never connects as `postgres`.** Functions are written in Step 4 with the authentication flow; the seam is documented now so it cannot be filled in ad hoc. |
...
| # | Conflict | Resolution |
|:--:|---|---|
| **C-19** | **The lockout was evaluated against the wrong clock.** `login_attempts.created_at` is written by `now()` on the **database**, and `evaluateLockout()` was being handed `Date.now()` from the **application**. `evaluateLockout` discards future-dated attempts as a safety measure — so with the database ahead, every freshly recorded failure looked like it was in the future and was thrown away. **The counter never reached three and the lockout silently never tripped.** Measured skew between this machine and Supabase: **22 seconds.** | ✅ **`getLockoutInputs()` returns the database's own `now()`, and the caller must use it.** Both sides of every comparison now come from the same source, so the result does not depend on two machines agreeing. `Date.now()` remains correct for what the app itself times — Argon2 decisions, temporary-password expiry, session lifetimes.<br><br>**Found by the Gate 4 integration test, not by review.** Two assertions failed; the natural reading was "my test is wrong", and it was not. A control that fails **open** under a few seconds of ordinary clock drift is not a control — and nothing in the unit suite or the SQL gate could have caught it, because each proves its own side in isolation and this lived precisely at the seam. |

| **C-20** | **Doc 04 §2 specifies a `statuses` table; `lib/domain/constants.ts` already holds the same eight rows as `STATUS_META`.** The table carries `colour`, `sort_order`, `category` and `counts_toward_load` — every one of which is already in `STATUS_META`, which registry §4 names the single source of truth for enums. Building both would give the capacity engine two places to read `loadWeight` from. | ✅ **The *set* of statuses is a Postgres enum (`public.task_status`, migration 012); the *metadata* stays in `STATUS_META`.** The database enforces which statuses can exist; TypeScript holds what each one means, in one place, unit-tested. This is the same reasoning that produced C-16 — a second hand-maintained declaration of the same facts is guaranteed drift — applied to statuses instead of to the schema.<br><br>**What is given up:** statuses are not runtime-editable by a Super Admin. Doc 19 §5 lists `status_workflow` as a setting, so that remains a real gap; adding a status is a migration plus a constants edit. Accepted deliberately: an eight-status workflow whose weights feed every capacity figure is not something to change without a code review, and doc 05 §2's transition table would have to be edited by hand anyway. |
| **C-21** | **Doc 03 §4 hides nav items by role; nothing stopped a Member reaching the page by URL.** `sectionsForRole()` omits Team, Workload, Reports and Settings from a Member's sidebar — and its own comment says "hiding a nav item is convenience, never security (NFR-006)". Nothing was enforcing the other half. Row-level security meant a Member saw only their own row, so nothing leaked, but they reached a screen built to answer a question they are not entitled to ask. | ✅ **`requireRole(minimum)` on each page, mirroring nav-config's role lists.** Redirects rather than showing a 403 — there is nothing useful to do with a refusal and their own starting screen is one navigation away.<br><br>**Found by `npm run smoke`, which fetches every route as a seeded Member.** Neither a build nor a click-through could have found it: the sidebar never offers the link, so nobody would click it. The script now asserts the inversion too — below a route's floor, a redirect is the pass and a 200 is the failure. |
...
| Table | Change | Why |
|---|---|---|
| *all* | `organisation_id` **not created** | [ADR-008](decisions/ADR-008-single-tenant.md), FR-162 dropped. Doc 04 §2 still lists it on `users` and `projects`; those rows are void. |
| `users` | `+ locked_at timestamptz` | FR-155a's lock must auto-clear after 30 minutes (`accountLockAutoClearMinutes`). `account_state = 'locked'` records *that* it is locked; nothing recorded *when*. The failed-attempt **count** stays derived from `login_attempts` — never stored. |
| `users` | `+ created_by_id`, `+ last_login_at` | The provisioning chain (FR-141) is only auditable if each row records who created it. |
| `users` | `email` forced lowercase by check constraint | Two accounts differing only in case would be two identities for one person. |
| `users` | **at most one `super_admin` row, enforced by a partial unique index** | BR-028 — "no in-app path creates a Super Admin" made structural rather than procedural. Lifting it is one `DROP INDEX` in a reviewed migration, which is what doc 16 §6's sealed procedure would be. |
| `auth_identities` | `+ unique (user_id, provider)` | One password identity and one Google identity per person (FR-160). |
| `invitations` | `+ account_unlock` purpose (C-17), `+ attempt_count`, `+ invalidated_at` | FR-155: max 5 code attempts, and requesting a new code invalidates the previous one. |
| `invitations` | `token_hash` constrained to `^[0-9a-f]{64}$` | Makes "the raw token is never stored" a **database** invariant, not a promise in a code review. |
| `sessions` | `+ rotated_from_id`, `+ reuse_detected_at` | FR-150 requires rotation *with reuse detection*; detection needs the chain and a place to mark the breach. |
| `mfa_factors` | `+ sign_count` | WebAuthn replay protection — the authenticator counter must be persisted and monotonic. |
| `audit_log`, `security_events` | `actor_id` / `user_id` carry **no foreign key**; `actor_email` and `actor_role` are **snapshotted** | An evidence log must stay true after the actor's row changes, and an `ON DELETE SET NULL` would be a system-performed mutation of an append-only row. Doc 03 §7's "purging anonymises history" is satisfied because the log holds a snapshot, not a live reference. |
| `system_settings` | **seeded with nothing** | The table holds *overrides only*. `getSetting(key)` returns the stored value or falls back to `SYSTEM_DEFAULTS` in `lib/domain/constants.ts`. Seeding would put the same 33 defaults in three places (§5 here, `constants.ts`, and the database) with no mechanism keeping them equal. Adding a setting now needs no migration. |
| `system_settings` | RLS is **coarse** (Super Admin and Admin may write; everyone reads) | §5's per-key `editable_by` precision lives in `lib/domain/permissions.ts` and the server action — RLS cannot express it without duplicating the matrix. Layer 2 of 2, deliberately the blunter one. |
| append-only tables | protected by a **trigger**, not only by revoked grants | Doc 19 §6 requires "no UPDATE or DELETE grant for **any** role". A grant cannot restrain the table **owner**; a trigger can. Both are applied. |
