...
| **Steps done in this batch** | **3.1a** migration 020 · **3.1b** task times · **3.1c** project times |
| **⏭️ NEXT ACTION** | Step 3.2 — the project form by TYPE: Event gets duration (single day → one date + start/end time; multi-day → start → due) plus venue and expected attendance; Client gets client name, retainer/one-off, contract end; Business gets area and sponsor; Self-promotion gets channel and goal. Stored in the existing `type_fields` jsonb — **no migration needed**. Then browser-verify 3.1c and 3.2 together. |
