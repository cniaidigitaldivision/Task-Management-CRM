...
| **⏭️ NEXT ACTION** | Step **4.1** — forced password reset with the status trail (sent → delivered → link opened → completed), plus **Resend** and **Revoke link**. Note before starting: `forceResetAction` currently sends **no email at all**, so 4.1 has to make the reset actually send one. "Link opened" is recorded server-side when `/reset-password` is hit with a valid code — no tracking pixel. |
