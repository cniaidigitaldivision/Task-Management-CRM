...
function AssignmentAccountability({
  ...
  /* ⚠️ FOUR, AND THE REPEAT ONE COMES FIRST. Without it, a person whose work is
     generated by a daily rotation reads as "100% self-created", which credits
     them with a decision the engine made. 596 of 1,241 tasks here. */
  const cards = [
    { icon: Repeat, label: 'Daily rotation', value: sources.repeat, tone: 'var(--pf-amber-bg)', ink: 'var(--pf-amber)' },
        ...
        {sources.total === 0 ? (
          ...
          <>
            ...
            <p className="mt-[0.7rem] text-[0.8rem] leading-[1.45]" style={{ color: 'var(--pf-faint)' }}>
              A task generated by a repeat is counted as a rotation, not as anybody&rsquo;s decision
              that day. The rest is read from who raised it, against that person&rsquo;s rank today
              &mdash; a coordinator later promoted would make their older assignments read as
              admin-assigned.
            </p>
...
function sourceHelp(h: HistoryEntry): string {
  ...
  if (h.sourceKind === 'repeat')
    return `Created automatically by a ${repeatWord(h.recurrenceRule).toLowerCase()} repeat, not raised by a person that day.`;
...
function sourceLabel(h: HistoryEntry): { text: string; tone: string } {
  /* ⚠️ A REPEAT COPY IS NOT SOMEBODY'S DECISION THAT DAY, so it is named before
     any of the people are. */
  if (h.sourceKind === 'repeat')
    return { text: `${repeatWord(h.recurrenceRule)} rotation`, tone: 'var(--pf-amber)' };
