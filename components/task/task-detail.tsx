...
export function TaskDetail({
  ...
  /* The same verdict the server will reach — see `deleteRefusal`. Computed here
     so the control and the action cannot disagree. */
  const deleteVerdict = data.task
    ? deleteRefusal(
        { id: currentUser.id, role: currentUser.role },
        {
          createdById: data.task.createdById,
      ...
      >
        ...
        {task && (
          <div className="space-y-5">
            ...
            {/* ── ⚠️ WHERE THIS TASK CAME FROM ─────────────────────────────────
                Owner, 2026-08-23: *"when I click on a task detail, it is not
                showing whether this task is generated by the user or by someone
                else… when they click on a task they should know whether this
                task is generated by themselves or it is assigned by someone
                else. This should be clearly defined."*

                It WAS shown — as "Raised by · Kashif Ahmed", one of four small
                grey tiles below the fold, printing a name with no indication of
                whether that name is you. Which is to say it answered the
                question only for somebody who already knew the answer.

                This states the relationship instead of the fact, in the second
                person, above the detail rather than under it. The four tiles
                keep the raw name for the case where somebody wants to read it
                off precisely. */}
            <OriginBanner
            ...
            <div className="flex flex-wrap items-center gap-2 border-t border-border-subtle pt-4">
              ...
              {/* ── ⚠️ SHOWN ONLY WHEN THE RULE ALLOWS IT ────────────────────
                  `deleteRefusal` is the same function the action calls, so the
                  button and the server cannot disagree about whether this is
                  deletable. It was unconditional, which meant a Coordinator
                  could press Delete on a Done task somebody else raised and be
                  refused twice over — once for each rule they had not been told
                  about. */}
              {deleteVerdict === null ? (
                <>
                  <Button
                ...
                /* Says why rather than hiding silently. A control that vanishes
                   with no explanation reads as a bug; a sentence naming the rule
                   is the rule doing its job. */
                <span className="text-micro text-text-tertiary">
                  {deleteVerdict === 'not_yours'
                    ? `Only ${task.createdByName ?? 'whoever raised this'} or an Admin can delete this task.`
                    : `Past ${STATUS_META[task.status].label} — only an Admin can delete this now.`}
                </span>
              )}
            </div>
          </div>
...
/* ============================================================================
 * WHO PUT THIS TASK HERE
 * ----------------------------------------------------------------------------
 * Four relationships, four sentences. The distinction that matters to the person
 * reading it is not "who created this row" but "is this mine because I chose it,
 * or because somebody gave it to me" — and those have different consequences:
 * a task you raised yourself, you may also delete (`task.soft_delete` is
 * `self_created` for a Member); one handed to you, you may not.
 *
 * ⚠️ Written in the second person deliberately. "Raised by Kashif Ahmed" makes
 * the reader compare a string against their own name; "Kashif Ahmed assigned
 * this to you" does not.
 * ========================================================================= */
function OriginBanner({
  ...
  const mine = createdById === viewerId;
  const forMe = assigneeId === viewerId;

  const { icon: Icon, text, tone } = mine
    ? forMe
      ? {
          icon: UserCircle2,
          tone: 'var(--accent-primary)',
          text: 'You created this task for yourself.',
        }
      : {
          icon: UserCircle2,
          tone: 'var(--accent-primary)',
          text: assigneeName
            ? `You created this task and assigned it to ${assigneeName}.`
            : 'You created this task. Nobody is assigned to it yet.',
        }
    : forMe
      ? {
          icon: UserPlus,
          tone: 'var(--accent-gold)',
          text: `${createdByName ?? 'Somebody'} assigned this task to you.`,
        }
      : {
          icon: UserPlus,
          tone: 'var(--text-secondary)',
          text: assigneeName
            ? `${createdByName ?? 'Somebody'} created this task for ${assigneeName}.`
            : `${createdByName ?? 'Somebody'} created this task. Nobody is assigned to it yet.`,
        };
  ...
  return (
    <p
      ...
      style={{
        backgroundColor: `color-mix(in oklab, ${tone} var(--tint-soft), var(--bg-surface))`,
        border: `1px solid color-mix(in oklab, ${tone} 28%, transparent)`,
        color: 'var(--text-primary)',
      }}
    >
      <Icon className="size-4 shrink-0" style={{ color: tone }} aria-hidden="true" />
      {text}
    </p>
  );
}
