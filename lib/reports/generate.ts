...
import { insertProjectReport } from '@/lib/db/queries/report-files';
...
/**
 * Who a report is generated as.
 *
 * Structurally satisfied by `CurrentUser`, so the action passes its session user
 * straight through with no adapter -- and buildable from a `users` row, which is
 * what lets the cron run a schedule as its author.
 */
 ...
 * It takes its actor as an argument, and in a `'use server'` file every
 * exported async function becomes an endpoint the browser may call with
 * arguments of its choosing. An exported `generate(actor, ...)` there would let
 * anybody generate a report AS anybody -- the actor is what RLS and `can()` are
 * both keyed on. So it lives here, in a `server-only` module, with exactly two
 * callers:
 *
 *   generateProjectReportAction  -- somebody pressed the button; the actor is
 *       whoever the session says they are.
 *   lib/reports/schedule-runner  -- the cron; the actor is the person who set
 *       the schedule up, read from the database.
 *
 * -- MOVED HERE VERBATIM, ON PURPOSE ----------------------------------------
 * This body is unchanged from `app/actions/project-report.ts`, down to the local
 * variable still being called `user`, because the owner approved this document's
 * every panel over three rounds of screenshots and a tidy-up during the move is
 * how a figure quietly changes. The only edit is the signature: the session read
 * became a parameter.
 */
export async function generateProjectReport(
  /* The same fields the action read off the session and no more, narrowed so the
     cron can build one from a `users` row without inventing a session.
     `auditAlone` needs `id`, `email` and `role`; the footer needs `fullName`. */
  user: ReportActor,
  projectId: string,
  kind: string,
  from?: string,
