/* ============================================================================
 * PLATFORM BRAND MARKS — the data, with no React around it
 * ----------------------------------------------------------------------------
 * These paths were extracted from `simple-icons` (CC0) for `PlatformIcon`. They live
 * here rather than in that component for one reason: the report composer draws the same
 * marks into a PDF **on the server**, and `components/brand/platform-icon.tsx` carries
 * `'use client'`. Importing a client module from a server module is a boundary error,
 * and copying two thousand characters of path data into a second file is how the two
 * quietly stop matching.
 *
 * ⚠️ Nothing here may import React or anything from `components/`. It is data.
 *
 * ⚠️ LinkedIn is not in `simple-icons` — it was removed at LinkedIn's request — so its
 * mark is described as geometry (`LINKEDIN_SHAPES`) rather than a path. Both renderers
 * consume that description; neither reconstructs it.
 *
 * ⚠️ The keys are the `slug` column of `public.platforms`, not display names. Names get
 * reworded — "X (Twitter)" already has — and a lookup keyed on a label silently loses
 * its icon the day somebody edits it.
 * ========================================================================= */
...
export interface PlatformMark {
  ...
  readonly hex: string;
  /** Whether the mark knocks out light or dark against `hex`. */
...
export const PLATFORM_MARKS: Readonly<Record<string, PlatformMark>> = {
  ...
  /* ⚠️ Drawn, not extracted — see the header. */
  linkedin: {
    label: 'LinkedIn',
    hex: '#0A66C2',
    glyph: 'light',
...
/**
 * The LinkedIn "in" wordmark as geometry in the same 24×24 box: a dot and stem for the
 * i, a stem and arch for the n. Shared so the DOM renderer and the PDF renderer draw
 * the same approximation rather than two different ones.
 */
export const LINKEDIN_SHAPES = {
  dot: { cx: 5.4, cy: 5.1, r: 2.4 },
  stem: { x: 3.2, y: 9.2, width: 4.4, height: 11.6, rx: 0.5 },
  arch:
    'M10.4 9.2h4.2v1.6a4.6 4.6 0 0 1 3.9-1.9c3 0 4.3 1.9 4.3 5.2v6.7h-4.4v-6c0-1.5-.5-2.4-1.8-2.4-1.2 0-1.9.8-1.9 2.4v6h-4.3z',
} as const;

/** Instagram's real icon is a gradient, and flat magenta reads as the wrong app. */
export const INSTAGRAM_GRADIENT =
  'linear-gradient(45deg, #F58529 0%, #DD2A7B 45%, #8134AF 72%, #515BD4 100%)';

/**
 * Instagram's gradient as discrete stops. The PDF renderer has no gradient primitive,
 * so it draws these as bands — see `report-poster.ts`. Kept beside the CSS gradient so
 * the two cannot drift apart.
 */
export const INSTAGRAM_STOPS: readonly string[] = [
  '#F58529',
  '#DD2A7B',
  '#8134AF',
  '#515BD4',
...
export function platformMark(slug: string): PlatformMark | null {
  return PLATFORM_MARKS[slug] ?? null;
}

/**
 * The slug for a platform display name.
 *
 * ⚠️ Needed only by the report composer, which is handed platform NAMES (they are what
 * goes on the page) but needs slugs to pick a mark. Everywhere else in the application
 * the slug is already to hand and this must not be used — see the header on why keying
 * off a label is fragile. Matching is loose on purpose: "X (Twitter)" has to find `x`.
 */
export function slugForPlatformName(name: string): string | null {
  const needle = name.trim().toLowerCase();
  if (needle.length === 0) return null;

  for (const [slug, mark] of Object.entries(PLATFORM_MARKS)) {
    if (mark.label.toLowerCase() === needle) return slug;
  }
  /* Then the slug itself, then a prefix — "X (Twitter)" starts with "x". */
  for (const slug of Object.keys(PLATFORM_MARKS)) {
    if (slug.replace(/_/g, ' ') === needle) return slug;
  }
  for (const [slug, mark] of Object.entries(PLATFORM_MARKS)) {
    const label = mark.label.toLowerCase();
    if (needle === label || needle.startsWith(`${label} `) || needle.startsWith(`${label}(`)) {
      return slug;
    }
  }
  return null;
